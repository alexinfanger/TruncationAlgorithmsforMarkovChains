...
out_lin = CS_tv_bounds(K_union, Aidx, R, tilde_lin_S, h1_S, h2_S, row_normalize, e_S = tilde_e_S)
...
pcmc = PrecomputedToggleSwitch(mc, 
        piv, out_e.tvb, "TS-20-1", 
        Dict(eq_exp_e.name=>eq_exp_e, 
                 eq_exp_lin.name=>eq_exp_lin))

save("src/models/toggle_switch/data/TS-20-1.jld2",
      Dict("pcmc"=>pcmc))
