...
n_2 = ceil(2*pcmc.mc.br+4*(pi_mode[1]-1)+1)
n_union = max(n_1,n_2)
n_mb = ceil((α*c1 +sqrt(α^2*c1^2+4*(α*c2/2-c3)*α*c0))/(2*(α*c2/2-c3)))
...
r0tvals=[100,110,120,130,140,150]
...
g_3_S = α*sq_c_S
mb_val = LyapunovBound2MomentBoundQ_on_subset(Q, g_3_S, sq_sp_S, A_mb)
b_mb = @benchmark LyapunovBound2MomentBoundQ_on_subset($Q, $g_3_S, $sq_sp_S, $A_mb)

println("confirm moment bound holds: ", mb_val>= pcmc.piv'*sq_sp_S)

SDP = (active = false, bound = 1.8e7, tvals=r1tvals, w_S = [0.0], name="2-norm-square")
Lyapunov = (active = true, bound=mb_val,  tvals=r1tvals.^2, w_S = sq_S, name="Lyapunov-Next-Higher")
